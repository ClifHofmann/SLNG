...
- **SLNG Codebase:** Licensed under the [MIT License](LICENSE).
- **Third-Party Notices:** We ship a small amount of Second Life viewer artwork (the terrain blend ramp and the Windlight cloud texture) under **Creative Commons Attribution-Share Alike 3.0**, Windlight XML presets under **LGPL 2.1**, LibreMetaverse under **BSD 3-Clause**, and Godot Engine under **MIT**. The canonical notice file is [`app/THIRD-PARTY-NOTICES.md`](app/THIRD-PARTY-NOTICES.md) (and pointed to by [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md)), which is also displayable in-client under **Preferences → Licences**.
- **Third-Party Viewer (TPV) Policy:** Any build that connects to the Second Life grid must comply with the Linden Lab **TPV Policy**. The permission and asset-protection model is a hard design constraint, not an afterthought — see [AGENTS.md](AGENTS.md#non-negotiables).
