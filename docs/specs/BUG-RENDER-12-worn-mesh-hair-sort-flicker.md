...
- Mesh hair no longer switches which cards are in front as the camera orbits an avatar. ✅ v0.21.15
- Strand tips read as soft, not hard-cut spikes, and the hair stays opaque. 🧪 v0.21.18
- Hair does not become a solid opaque sheet at any zoom level.
- BoM skin under one *and* two alpha-layer wearables is unchanged (BUG-RENDER-09).
- Veils / tinted translucent panes still blend.

## Correction — the edge band was set wrong (v0.21.16 → v0.21.17)
...
## Calibration — the band width, with both ends now measured (v0.21.18)
...
cause directly rather than the shading: the hair's **six faces per mesh all carry the same texture
and tint** (`face ids: [b9af3b5f × 6]`, `default=b9af3b5f`). Merging same-material faces into one
surface would take the scene from **18 sorted transparent draws to 3**, which would make `Kind.Blend`
— real translucency, the Firestorm look — viable again. That is a mesh-build change, not a number.

## Final approach — port the viewer's design instead of working around it (v0.21.19)

The cutout route (v0.21.14–0.21.18) removed the flicker but traded away the softness, and tuning
the coverage band only moved the trade-off along one axis: `0.00` too hard, `0.42` too transparent.
It was a workaround, not a port. Reading the viewer's actual sorting code settled what the port
should be, and it is not a cutout at all.
