...
## Handover to Gemini (2026-07-22, end of this Claude session)

**Branch**: `fix/remote-avatar-animations`, `main` worktree (E:\Git\SLNG). Tip commit `82d24a5` as of this handover — check `git log --oneline main..HEAD` for the current count (21 commits ahead of `main` at time of writing). `AppVersion` at `v0.1.97-alpha`. `git status` should be clean except this file; if it isn't, read the diff before touching anything (someone may be mid-edit).

**What's solid and confirmed working, live, multiple times over, across this whole multi-round investigation**:
- The original bug (avatars — local AND remote — floating or sinking relative to the ground) is fixed for the general case. Root causes were a chain of independent bugs, documented in full in rounds 2-9 above: an `AgentId` race dropping remote avatars' appearance data, a later regression re-opening that same race mid-session, a self-aliasing `Dictionary.Clear()` silently resetting shape-derived height constants to default, a wrong assumption about network-position semantics (pelvis vs. capsule-center — verified against real `linden_llvoavatar.cpp` source), and a missing `AppearanceHover` term for remote avatars.
- The "Reamon renders with wrong proportions vs. Clifton" complaint is confirmed NOT a separate bug — `[DBG-PARITY]`'s per-bone check (tagged per-entity, rounds 8-9) shows both avatars' rendered skeletons match their own shape data to within 1cm/1%. It was a visual artifact of the (then-broken) position bug, nothing more.

**What's still open, in priority order**:
1. **A second, just-fixed "Hover" term (round 11, commit `82d24a5`) — NOT YET LIVE-VERIFIED.** Found a completely separate hover mechanism (`AVATAR_HOVER`, VisualParam id `11001`, the Body-shape editor's "Hover" slider) distinct from the `AppearanceHover` network field round 9 already fixed — the real viewer applies both, at different points in the formula, and this renderer had never read this specific VisualParam at all before this commit. Committed and build-verified, but **not yet confirmed against a live client** — this session ran out of ability to test further (no GUI automation for the native Godot window in this environment throughout the whole investigation; every live check needed a human in the loop). **This should be the very first thing checked in the next session**: rebuild clean, fresh relogin, get Reamon (or any remote avatar known to be sinking/floating) in view, and read `[HeightDebug]`'s new `avatarHoverParamZ` field per the "Next step" instructions immediately above this section. If it closes the residual ~5-8cm gap, this investigation is genuinely done. If not, the doc above has the fallback candidates to chase next.
2. **Low-priority, demoted thread**: both avatars measure ~14-15cm under their Firestorm-displayed height stat (2.22m Clifton, 2.19m Reamon vs. our ~2.05-2.07m `renderedSkullToFootY`), despite our `ComputeBodySize` formula and the raw wire-transmitted `Scale.Z` agreeing with each other almost exactly — ruling out two hypotheses already checked (round 10). Not urgent; the visually-broken bug is fixed regardless of this discrepancy. Candidate explanations documented in round 10's final section.
3. **Known, narrower residual limitation from round 5**: if two remote avatars are simultaneously unresolved (`AgentId == Guid.Empty`) at the exact same instant, `WorldSimulation.FindAvatarEntityByAgentId`'s fallback still can't disambiguate which one a given `AvatarAppearance`/`AvatarAnimation` event is about (no `LocalId`/region hint carried on those events). Narrow window, not observed as a live symptom this session, but worth knowing the gap exists if cross-avatar data contamination is ever seen again.

**Process lessons from this session, worth internalizing before continuing** (this took 11 rounds; these are why):
- This project's diagnostics-first discipline paid off repeatedly: several rounds where "the numbers still don't match" turned out to be bugs in a *diagnostic's own throttle logic* (rounds 4-5), not the underlying system being investigated. Don't trust the absence of expected log output as evidence of anything without first confirming the diagnostic itself actually fires.
- A "the math checks out" confirmation can be a tautology if both sides of the comparison are code you wrote yourself. This bit the investigation more than once — see the `Root.Y == groundHeight` self-cancelling trap on the now-merged `fix/avatar-pelvis-offset` branch (referenced early in this doc), and implicitly again in how the local avatar's own formula/clamp pairing was never independent evidence the formula itself was right. The remote avatar (external, server-controlled ground truth) was the only genuine test throughout — always reason from the side of the equation you DON'T control.
- Screenshots are useful for spotting problems but not sufficient for final confirmation — round 10 closed this exact bug based on a screenshot that, on closer inspection, still showed it. Cross-check against the actual log numbers before declaring victory, every time, especially for small residual gaps (a few cm) that are easy to miss at a glance or from a favorable camera angle.
- When two AI sessions hand a bug back and forth, re-read the FULL history before acting — several rounds in this file corrected an earlier round's incomplete or wrong conclusion. The most recent round's status is not automatically the last word; check the actual live log numbers, not just what the last summary claimed.
