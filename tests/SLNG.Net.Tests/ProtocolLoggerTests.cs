...
public class ProtocolLoggerTests
{
    ...
    public void ProtocolLogger_FormatsObjectUpdatesCorrectly()
    {
        ...
        var objEvent = new ObjectUpdateEvent(42, new Vector3(10f, 20f, 30f), new Quaternion(0, 0, 0, 1));
        session.RaiseObjectUpdate(objEvent);

        var output = sb.ToString().Trim();
        var expectedPos = new Vector3(10f, 20f, 30f).ToString();
        var expectedRot = new Quaternion(0, 0, 0, 1).ToString();
