...
public class AvatarShapeServiceTests
{
    ...
    public void Raw_weight_zero_means_zero_distortion_for_that_param()
    {
        ...
        if (distortions.TryGetValue("mNeck", out var neck))
        {
    ...
    public void Param_at_its_own_default_value_applies_that_default_as_distortion()
    {
        ...
        Assert.True(distortions.TryGetValue("mNeck", out var neck), "expected mNeck to have a distortion entry");
    ...
    public void Sex_gating_reads_avatar_lad_xml_without_changing_unrelated_bones()
    {
        ...
        Assert.Equal(withoutCharDir.Count, withCharDir.Count);
        foreach (var (bone, dist) in withoutCharDir)
        {
            Assert.True(withCharDir.TryGetValue(bone, out var dist2));
    ...
    public void Driven_param_head_size_reaches_mFaceRoot()
    {
        ...
        Assert.True(distortionsAtMax.TryGetValue("mFaceRoot", out var faceRootAtMax), "expected mFaceRoot to have a distortion entry from the driven \"Head Size\" param");
        distortionsAtMin.TryGetValue("mFaceRoot", out var faceRootAtMin); // may be absent (zero) if nothing else touches mFaceRoot here
